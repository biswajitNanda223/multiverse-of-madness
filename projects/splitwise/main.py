...
app = FastAPI(
    ...
    description=(
        "Low-Level Design implementation of Splitwise with Equal, Exact, "
        "and Percentage Split strategies and Debt Simplification algorithm."
    ),
    version="1.0.0",
)
...
class AddExpenseRequest(BaseModel):
    ...
    split_type: SplitType = Field(..., description="Type of split: EQUAL, EXACT, or PERCENT")
...
def create_group(request: CreateGroupRequest) -> dict:
    try:
...
def add_group_member(group_id: str, request: AddMemberRequest) -> dict:
    try:
...
def add_expense(group_id: str, request: AddExpenseRequest) -> dict:
    try:
...
def get_group_balances(group_id: str) -> dict:
    try:
...
def simplify_group_debts(group_id: str) -> dict:
    try:
